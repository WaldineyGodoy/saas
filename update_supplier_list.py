...
imports = """import { useState, useEffect, useMemo } from 'react';
...
with open('src/pages/dashboards/SupplierList.jsx', 'w', encoding='utf-8') as f:
    f.write(imports + "\n\n")
    # Now append the rest of the component
    lines = content.split('\n')
    inside_component = False
    new_lines = []
    
    for line in lines:
        if line.startswith('export default function SupplierList()'):
            inside_component = True
        if inside_component:
            new_lines.append(line)
            
    # Modify states and add viewMode
    comp_code = '\n'.join(new_lines)
    
    comp_code = comp_code.replace("const [searchTerm, setSearchTerm] = useState('');", "const [searchTerm, setSearchTerm] = useState('');\n    const [viewMode, setViewMode] = useState('kanban');\n    const [activeId, setActiveId] = useState(null);")
...
"""
    comp_code = comp_code.replace("const handleSave = () => {", drag_logic + "\n    const handleSave = () => {")
    
    # Replace the view rendering logic
    # Add buttons
    toolbar_replacement = """
            {/* Toolbar */}
            <div style={{ 
...
"""
    # Use regex to replace the toolbar section up to the end of the div
    comp_code = re.sub(r'\{\/\* Toolbar \*\/\}.*?Novo Fornecedor\s*</button>\s*</div>\s*</div>', toolbar_replacement, comp_code, flags=re.DOTALL)
    
    # Replace list rendering logic
    list_rendering = """
            {loading ? (
...
"""
    comp_code = re.sub(r'\{loading \? \(.*?</div>\s*\)\s*\}\s*\{\s*isModalOpen\s*&&\s*\(', list_rendering + '\n\n            {isModalOpen && (', comp_code, flags=re.DOTALL)
    
    # Let's also fix the metric cards
    cards_replacement = """
            {/* Header Cards */}
            <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: '220px', background: 'white', padding: '1.2rem', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                        <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.025em' }}>Total</div>
                        <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#1e293b' }}>{suppliers.length}</div>
                    </div>
                </div>
                {KANBAN_STATUSES.map(st => (
                    <div key={st.status} style={{ flex: 1, minWidth: '220px', background: 'white', padding: '1.2rem', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <div style={{ padding: '0.8rem', background: st.bg, borderRadius: '12px' }}>
                            <Building2 size={24} color={st.color} />
                        </div>
                        <div>
                            <div style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.025em' }}>{st.label}</div>
                            <div style={{ fontSize: '1.5rem', fontWeight: 800, color: st.color }}>{suppliers.filter(s => s.status === st.status).length}</div>
                        </div>
                    </div>
                ))}
            </div>
"""
    comp_code = re.sub(r'\{\/\* Header Cards \*\/\}.*?\{\/\* Toolbar \*\/\}', cards_replacement + '\n            {/* Toolbar */}', comp_code, flags=re.DOTALL)
    
    # Clean up statusColors array as we replaced it with KANBAN_STATUSES
    comp_code = re.sub(r'\s*// Status colors mapping.*?};\n', '', comp_code, flags=re.DOTALL)
